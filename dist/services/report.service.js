...
exports.reportService = {
    ...
    userRegistrations: async (startDate, endDate) => {
        ...
        return users.reduce((acc, user) => {
            const month = new Intl.DateTimeFormat("en-US", { month: "short" }).format(user.createdAt);
    ...
    verifiedUsers: async (startDate, endDate) => {
        const users = await prisma.user.findMany({
            where: {
                isVerified: true,
                createdAt: {
    ...
    salesSummary: async (startDate, endDate) => {
        ...
        return orders.reduce((acc, order) => {
            const month = new Intl.DateTimeFormat("en-US", { month: "short" }).format(order.createdAt);
            if (!acc[month])
                acc[month] = { totalSales: 0, vat: 0, totalOrders: 0 };
    ...
    orderStatus: async (startDate, endDate) => {
        const orders = await prisma.order.findMany({
            where: {
                createdAt: {
                    gte: new Date(startDate ?? "2025-01-01"),
    ...
    topProducts: async (startDate, endDate) => {
        ...
        return products.reduce((acc, product) => {
            const month = new Intl.DateTimeFormat("en-US", { month: "short" }).format(product.createdAt);
    ...
    transactionSuccessRate: async (startDate, endDate) => {
        ...
        const formattedData = transactions.reduce((acc, transaction) => {
            const month = new Intl.DateTimeFormat("en-US", { month: "short" }).format(new Date(transaction.createdAt));
            if (!acc[month])
                acc[month] = { successful: 0, failed: 0, total: 0 };
    ...
    wishlistTrends: async (startDate, endDate) => {
        ...
        return wishlists.reduce((acc, wishlist) => {
            const month = new Intl.DateTimeFormat("en-US", { month: "short" }).format(wishlist.createdAt);
    ...
    technicianRegistrations: async (startDate, endDate) => {
        ...
        return technicians.reduce((acc, technician) => {
            const month = new Intl.DateTimeFormat("en-US", { month: "short" }).format(technician.createdAt);
            acc[month] = (acc[month] || 0) + 1;
            return acc;
        }, {});
    },
